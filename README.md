...
- Редирект на страницу `/match-oldScore?uuid=$match_id`

### Страница счёта матча - `/match-oldScore`

Адрес - `/match-oldScore?uuid=$match_id`. GET параметр `uuid` содержит UUID матча.
...
- Нажатие кнопок приводит к POST запросу по адресу `/match-oldScore?uuid=$match_id`, в полях отправленной формы содержится айди выигравшего очко игрока
...
- Обрабатывает POST запросы к `/match-oldScore`
